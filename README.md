# 👋 João Pedro Santos Batista

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=00D9FF&height=120&section=header&text=Full%20Stack%20Developer&fontSize=36&fontColor=fff&animation=fadeIn&fontAlignY=38&desc=Vue%20%7C%20React%20%7C%20Next.js%20%7C%20Node.js%20%7C%20TypeScript&descAlignY=60&descAlign=50" width="100%"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=00D9FF&center=true&vCenter=true&width=700&height=60&lines=🚀+Full+Stack+Developer;⚡+Vue+%7C+React+%7C+Next+%7C+Node;🧠+SaaS+%7C+PWA+%7C+Arquitetura+Web;🌟+Transformando+Ideias+em+Produtos" />

</div>

---

# 🎯 Sobre Mim | About Me

```ts
const joaoPedro = {
  role: "Full Stack Developer",
  experience: "1.5+ years",

  stack: {
    frontend: ["Vue", "React", "Next.js", "TypeScript", "Tailwind CSS"],
    backend:  ["Node.js", "Express", "BullMQ", "Redis", "Stripe"],
    database: ["MongoDB", "MySQL"],
    ai:       ["Anthropic API (claude-sonnet)"],
    devops:   ["Docker", "Linux"],
  },

  interests: [
    "SaaS Platforms",
    "Customizable Systems",
    "Automation & AI Layers",
    "Product Development",
  ],

  currentlyBuilding: [
    "Domus App (SaaS de gestão ministerial multi-tenant)",
    "LJ Cookies (plataforma de gestão de vendas artesanais)",
  ],

  motto: "Build useful things.",
}
```

- Desenvolvedor **Full Stack focado em produtos web modernos**
- Experiência com **Vue, React, Next.js, Node e APIs REST escaláveis**
- Construindo **SaaS multi-tenant e plataformas customizáveis**
- Interesse em **IA aplicada, automação e arquitetura distribuída**

---

# 🚀 Projetos em Destaque | Featured Projects

### Domus App

Plataforma **SaaS multi-tenant de gestão ministerial** — do CRM ao financeiro, da agenda ao onboarding.

Features:

- CRM com filtros compostos e segmentação dinâmica
- Agenda com Stripe Payment Intents e check-in de presença
- Automações via BullMQ (aniversários, ausências, eventos)
- Três camadas de IA: Analytics AI, Assistente Organizacional e Pastoral AI
- Relatórios em PDF por tier de plano (Puppeteer)
- Onboarding wizard e sistema de missões
- Quatro tiers de assinatura via Stripe Billing

Stack:

```
Node.js · Express · TypeScript · MongoDB
React 18 · Vite · Tailwind CSS · shadcn/ui
Stripe · BullMQ · Redis · Anthropic API · Docker
```

---

### LJ Cookies

Sistema completo de gestão para **produção e vendas de cookies artesanais**, desenvolvido para uso real.

Features:

- controle de estoque e compras
- registro de vendas e dashboard financeiro
- sistema de notificações
- sugestão de receitas baseada no estoque
- PWA com funcionamento offline

Stack:

```
Vue · Node.js · MySQL · PWA · Docker
```

---

# 📊 GitHub Analytics

<div align="center">

<img height="180em" src="https://streak-stats.demolab.com?user=jpsbat&theme=radical&hide_border=true"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=jpsbat&layout=compact&langs_count=8&theme=radical&hide_border=true"/>

</div>

---

# 🛠 Tech Stack

### **Frontend**

![Vue.js](https://img.shields.io/badge/Vue.js-35495E?style=for-the-badge&logo=vue.js&logoColor=4FC08D)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### **Backend**

![Node.js](https://img.shields.io/badge/Node.js-43853D?style=for-the-badge&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-404D59?style=for-the-badge)
![BullMQ](https://img.shields.io/badge/BullMQ-FF0000?style=for-the-badge&logo=redis&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)
![Stripe](https://img.shields.io/badge/Stripe-626CD9?style=for-the-badge&logo=stripe&logoColor=white)

### **Database**

![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-00000F?style=for-the-badge&logo=mysql&logoColor=white)

### **AI & Automation**

![Anthropic](https://img.shields.io/badge/Anthropic_API-191919?style=for-the-badge&logo=anthropic&logoColor=white)

### **DevOps & Tools**

![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

---

# 🌐 Conecte-se Comigo | Connect with me

<div align="center">

<a href="https://joaopedrobatistadev.com.br">
<img src="https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=todoist&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/joaopsbatista">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>
</a>

<a href="mailto:joaopedrobatista.dev@gmail.com">
<img src="https://img.shields.io/badge/Gmail-333333?style=for-the-badge&logo=gmail&logoColor=red"/>
</a>

</div>

---

<div align="center">

Obrigado pela visita! | Thanks for your visit!

</div>
